...
class MeteredResponse(Response):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 0 < content_length:
            librato.count('blueprint-acmeaws-com-server.bandwidth.out', content_length)
            statsd.update('blueprint-acmeaws-com-server.bandwidth.out', content_length)
...
def get_blueprint(secret, name):
    ...
    if browser_pattern.search(request.environ.get('HTTP_USER_AGENT', '')) \
       or 'html' == request.args.get('format'):
        librato.count('blueprint-acmeaws-com-server.renders')
        statsd.increment('blueprint-acmeaws-com-server.renders')
...
def put_blueprint(secret, name):
    ...
    librato.count('blueprint-acmeaws-com-server.bandwidth.in', request.content_length)
    statsd.update('blueprint-acmeaws-com-server.bandwidth.in', request.content_length)
